...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    # Configuration specific to ISO installer builds via
    # .#nixosConfigurations.<SYSTEM>.config.system.build.images.iso-installer
    image.modules = {
      iso-installer =
        { config, lib, ... }:
        {
        ...
        programs.waybar = {
          ...
          settings = {
            mainBar = {
              ...
              # Not having an output configures just defaults to outputting on all displays, including
              # new ones hotplugged at runtime.
              #
              # output = [
              #   "${cfg.primaryDisplayOutput}"
              # ];
